...
class StartingBoardState : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    board = std::make_shared<othello::GameBoard>(othello::createInitialBoard());
...
TEST_F(StartingBoardState, PossibleMovesInitialBoard) {
  ...
  EXPECT_THAT(possible_moves,
              ::testing::UnorderedElementsAreArray(expected_moves));
}

TEST_F(StartingBoardState, ApplyMove) {
  othello::GameBoard new_board =
      othello::applyMove(*board, 44, othello::Color::BLACK);
  othello::GameBoard expected_board = othello::createInitialBoard();
  ...
  expected_board = othello::createInitialBoard();
...
TEST_F(IntermediateBoardState, PossibleMovesIntermediateBoard) {
  ...
  EXPECT_THAT(possible_moves_white,
              ::testing::UnorderedElementsAreArray(expected_moves_white));
}
...
TEST_F(IntermediateBoardState, ApplyMove) {
  othello::GameBoard new_board =
      othello::applyMove(*board, 17, othello::Color::WHITE);
  othello::GameBoard expected_board = othello::createInitialBoard();
